...
def evaluate(model, dataloader, device,  out_dir=None):
    ...
    try:
        ...
        if preds.size and targets.size:
            ...
            metrics['accuracy'] = float(accuracy_score(targets, preds_bin))
            metrics['f1'] = float(f1_score(targets, preds_bin))
...
def main():
    ...
    val_base = all_mammo(args.val_csv, args.val_img_base, args.val_text_base, topk=args.topk, enable_augmentation=False)
